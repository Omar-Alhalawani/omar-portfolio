(() => {
  ...
  const context = canvas.getContext('2d'); let points = [], routes = [], pulse = null, nextPulse = 0, raf, last = 0;
  ...
  function draw(time) { if (document.hidden || document.body.classList.contains('game-active')) { raf = requestAnimationFrame(draw); return; } if (time - last > 32) { context.clearRect(0, 0, innerWidth, innerHeight); points.forEach(p => { p.x += p.vx; p.y += p.vy; if (p.x < 0 || p.x > innerWidth) p.vx *= -1; if (p.y < 0 || p.y > innerHeight) p.vy *= -1; }); routes.forEach(route => { context.strokeStyle = 'rgba(103,232,249,.065)'; context.beginPath(); context.moveTo(route.a.x, route.a.y); context.lineTo(route.b.x, route.a.y); context.lineTo(route.b.x, route.b.y); context.stroke(); }); points.forEach(p => { context.fillStyle = 'rgba(103,232,249,.3)'; context.fillRect(p.x - 1, p.y - 1, 2, 2); }); if (!pulse && time > nextPulse && routes.length) { pulse = { route: routes[Math.floor(Math.random() * routes.length)], start: time, hue: Math.random() > .86 ? '167,139,250' : '103,232,249' }; nextPulse = time + 2400 + Math.random() * 2600; } if (pulse) { const progress = Math.min(1, (time - pulse.start) / 850), point = routePoint(pulse.route, progress); context.fillStyle = `rgba(${pulse.hue},.9)`; context.shadowColor = `rgb(${pulse.hue})`; context.shadowBlur = 10; context.beginPath(); context.arc(point.x, point.y, 2.4, 0, Math.PI * 2); context.fill(); context.shadowBlur = 0; if (progress >= 1) pulse = null; } last = time; } raf = requestAnimationFrame(draw); }
